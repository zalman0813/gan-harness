...
description: Stage 1 — grill user intent + blindfold codebase research → produces specs/_batch/prd.md (per-batch PRD, all R as H2 sections) + specs/_batch/research.md (codebase facts with base_commit + timestamp). One human checkpoint post-grill.
argument-hint: "[intent dump or path to intent file] (optional — grill-master will ask if empty)"
model: opus
---
...
2. Phase 1 — Grill (spawn `grill-master` subagent; produces `specs/_batch/prd.md` + `specs/_batch/_research-queue.md`)
